...
export class HandTracker {
  constructor(onResults) {
    ...
    if (!HandsConstructor) {
      console.error('MediaPipe Hands not found. Available window keys:', Object.keys(window).filter(k => k.includes('hand') || k.includes('Hand') || k.includes('mediapipe')));
    ...
    this.hands.onResults((results) => {
      resultCount++;
      if (resultCount % 30 === 0) {
        console.log('MediaPipe onResults called:', resultCount, 'times. Hands detected:', results.multiHandLandmarks?.length || 0);
      }
      onResults(results);
    });
  }
